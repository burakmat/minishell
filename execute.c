...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/19 09:57:31 by bmat              #+#    #+#             */
/*   Updated: 2022/09/19 09:57:32 by bmat             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	new_process(t_shell *shell, t_node *node)
{
	...
	if (!pid)
	{
		set_input_redirections(node, \
			check_input_redirections(node));
...
void	builtin_process(t_shell *shell, t_node *node, int i)
{
	...
	while (1)
	{
		...
		else
		{
			...
			while (1)
			{
				...
				if (a < 0)
					break ;
				if (shell->exit_status != 127 && shell->exit_status_before \
					== 256 && builtin_check(node->exec_args[0]) == 0)
					shell->exit_status = 1;
			}
			break ;
		}
